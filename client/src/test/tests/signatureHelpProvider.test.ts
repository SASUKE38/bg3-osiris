...
suite("Signature Help Provider", () => {
	...
	test(
		"Signature help for builtin with multiple signature definitions and 0 current parameters should display the signature with the least parameters"
	);

	test(
		"Signature help for builtin with multiple signature definitions and current parameters should display the signature with the corresponding number of parameters"
	);
	...
	test("Signature help for signature with one signature definition should display the sole signature definition", async () => {
		const expectedDocumentation = new vscode.MarkdownString();
		expectedDocumentation.appendMarkdown(descriptionHeader + "\n");
		expectedDocumentation.appendMarkdown("Thrown when _Flag is set on _Speaker in the _DialogInstance.  \n");
		expectedDocumentation.appendMarkdown(examplesHeader);
		expectedDocumentation.appendCodeblock(
		...
		expectedDocumentation.appendMarkdown(seeAlsoHeader + "\n- GetFlag\n- FlagCleared");
	...
	test(
		"Signature help for signature with filled parameters and cursor on one of them should display with that parameter as active"
	);
	...
	test("Signature help for signature with filled parameters and cursor on next space should display with the next parameter to be filled as active", async () => {
		const expectedDocumentation = new vscode.MarkdownString();
		expectedDocumentation.appendMarkdown(descriptionHeader + "\n");
		expectedDocumentation.appendMarkdown(
