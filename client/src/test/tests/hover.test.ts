...
suite("Should do hover", () => {
	...
	test("Hover over left edge of builtin name should display wiki info", async () => {
		...
		await testHover(docUri, new vscode.Position(69, 0), 1, [{ contents: [expectedString], range: toRange(69, 0, 69, 7) }]);
	});
	...
	test("Hover over right edge of builtin name should display wiki info", async () => {
		...
		await testHover(docUri, new vscode.Position(81, 8), 1, [{ contents: [expectedString], range: toRange(81, 0, 81, 8) }]);
	});
