...
export class ComparisonAnalyzer extends AnalyzerBase {
		...
		function doAnalysis(node: ASTNode, thisArg: ComparisonAnalyzer) {
			for (const child of node.getNodeChildren()) {
				...
				if (child.kind === ASTNodeKind.COMPARISON_NODE) {
					thisArg.verifyNoStringLtGtComparison(res, child as ComparisonNode);
					thisArg.verifyNoBinaryOperationSameRhsLhs(res, child as ComparisonNode);
	...
	private verifyNoStringLtGtComparison(res: Diagnostic[], child: ComparisonNode) {
		...
		if (
			...
			this.nonEqualityOperators.find((value) => value === operator.value)
		) {
	...
	private verifyNoBinaryOperationSameRhsLhs(res: Diagnostic[], child: ComparisonNode) {
		const left = child.left;
		const right = child.right;

		if (
			this.sameTypeKinds.find((value) => value === left.kind && value === right.kind) &&
